...
ggplot(mpg, aes(x = factor(1), fill = factor(class))) +
  ...
  geom_text(data = percent,  # 上で作ったパーセントのデータ指定
    ...
    scale_y_continuous(labels = scales::percent) +   # y軸を%表記に
...
pbar +
  geom_bar(aes(fill = drv),
           position = "fill") +
    scale_y_continuous(labels = scales::percent) # 縦軸を%表記に
``` 
...
percent <-
  ...
  mutate(pct = n/sum(n)*100,
         pctp = scales::percent(n/sum(n))) # %付きの値も作成
...
labs <- str_c(percent$class, " (", percent$pctp, ")")
